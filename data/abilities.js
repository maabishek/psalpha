...
let BattleAbilities = {
  ...
  primalforce: {
    desc:
      "This Pokemon receives significantly reduced damage from supereffective attacks. Moongeist Beam, Sunsteel Strike, and the Mold Breaker, Teravolt, and Turboblaze Abilities cannot ignore this Ability.",
    shortDesc:
      "This Pokemon receives significantly reduced damage from supereffective attacks.",
    onSourceModifyDamage: function(damage, source, target, move) {
      if (move.typeMod > 0) {
        this.debug("Primal Force neutralize");
  ...
  flamingsoul: {
    ...
    onModifyPriority: function(priority, pokemon, target, move) {
      if (move && move.type === "Fire" && pokemon.hp === pokemon.maxhp)
        return priority + 1;
    },
    id: "flamingsoul",
    name: "Flaming Soul",
  ...
  domainexpansion: {
    shortDesc:
      "This Pokemon's moves nearly always goes first",
    onModifyPriority: function(priority, pokemon, target, move) {
      if (move && move.category === "Status", "Physical") {
        return priority + 3;
        move.ignoreAbility = true;
      }
    },
    ...
    evilsurge: {
    shortDesc:
      "This Pokemon's attacking stat is multiplied by 1.5 while using a Dark-type attack.",
    onModifyAtkPriority: 5,
    onModifyAtk: function(atk, attacker, defender, move) {
      if (move.type === "Dark") {
        this.debug("Evil Surge boost");
        return this.chainModify(1.5);
      }
    },
    id: "evilsurge",
    ...
    fullmonke: {
    shortDesc: "This Pokemon's Attack is raised by one stage",
    onModifyAtkPriority: 5,
    onModifyAtk: function(atk) {
      return this.chainModify(1.5);
    },
